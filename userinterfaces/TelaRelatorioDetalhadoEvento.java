...
public class TelaRelatorioDetalhadoEvento extends JFrame {
		...
		try (Connection conn = BancoDados.conectar()) {
			...
			if (inscricoes.isEmpty()) {
				...
				for (Inscricao inscricao : inscricoes) {
					...
					modelParticipantes.addRow(new Object[] { participante.getNomeCompleto(), participante.getEmail(),
	...
	private void exportarDados(Evento evento) {
		...
		fileChooser.setDialogTitle("Salvar como"); // Define o título da janela

		fileChooser.setSelectedFile(new File("relatorio_evento_participantes.xls")); // Nome padrão do arquivo

		int userSelection = fileChooser.showSaveDialog(this);  // Pra que o usuário escolha o local e o nome do arquivo

		if (userSelection == JFileChooser.APPROVE_OPTION) { // Verifica se o usuário clicou em "Salvar"
